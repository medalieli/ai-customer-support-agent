...
test("customer login is responsive and accessible", async ({ page }) => { await page.goto("/"); await expect(page.getByRole("heading", { name: /How can we help/ })).toBeVisible(); await expect(page.getByRole("tab", { name: "Customer" })).toBeVisible(); await expect(page.getByText(/sample customer and order information/)).toBeVisible(); expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true); });
